...
console.log('[E.I.O Engine] ✅ Motor v4.7.10 Ativo');
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
    if (alarm.name.startsWith('check_followback_')) {
        ...
        if (_activeAudits.has(username)) {
            console.log(`[E.I.O v4.7.10] ⏭️ Auditoria para @${username} já em execução, ignorando.`);
            return;
        }
        _activeAudits.add(username);

        console.log(`[E.I.O v4.7.10] ⏰ Auditoria de Segurança para @${username}...`);
        ...
        try {
            ...
            console.error('[E.I.O v4.7.10] Erro na auditoria de follow-back:', e);
...
self.eioMessageHandler = (message, sender, sendResponse) => {
    ...
    switch (action) {
        ...
        case 'bridge_connected':
            ...
            sendResponse({ success: true, version: '4.7.10' });
            break;
